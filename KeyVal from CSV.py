...
def keyval_from_csv(conn, script_params):
    ...
    for target_object in conn.getObjects(data_type, ids):
        ...
        try:        # this tries to automatically detect the used delimiter (can vary due to system config)
            delimiter = csv.Sniffer().sniff(file_handle.read(1024)).delimiter
            print("Using delimiter: ", delimiter)
        except Exception:   # might fail if the .csv is too short
        ...
        header = data[0]
        # get rid of the UTF-8 BOM in the A1 of the csv
